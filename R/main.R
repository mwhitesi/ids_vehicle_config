...
load <- function() {
  ...
  # Historical vehicles
  unhiDT = data.table::fread(here::here('../data/raw/qry_UN_HI_CARID_UNID_20190318.csv'), check.names = TRUE)
  ...
  histDT = data.table::fread(here::here('../data/raw/qry_UN_HI_CARID_full_20190318.csv'), check.names = TRUE)
...
filter_nonvehicles <- function(uDT) {
  uDT = uDT[!grepl('^XV', CARID)]
  uDT = uDT[!grepl('^C', CARID)]
  uDT = uDT[!grepl('^FRD', UNID)]
  uDT = uDT[!grepl('\\-\\d[XYVUROKJIGF]\\d+$', UNID, perl=TRUE)]
...
contrDT = unhiDT[!unhiDT[,CARID] %in% fleetDT[,AHS.Vehicle.ID] & HighestCount >= 5]
